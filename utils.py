...
def calculate_mse(distribution1, distribution2):
    ...
    mse = np.mean(
        [
            (distribution1[node] / total_samples - distribution2[node] / total_samples)
            ** 2
            for node in nodes
        ]
    )
...
def simulate_random_walks_until_convergence(
    G, initial_samples_count, steps, threshold=1e-4, min_steps=10
):
    """
    模拟带有收敛条件的随机游走过程
...
def samples_count_initializer(G: nx.Graph, total_samples: int) -> dict:
    """
    初始化样本分布
    """
    num_nodes = G.number_of_nodes()
    random_vec = np.random.rand(num_nodes)
    random_vec = random_vec / np.sum(random_vec)
    samples_count = {
        node: int(random_vec[i] * total_samples) for i, node in enumerate(G.nodes())
    }

    sum_without_last = np.sum(list(samples_count.values())[:-1])
    samples_count[list(samples_count.keys())[-1]] = total_samples - sum_without_last
    return samples_count


def Monte_Carlo_particle_transition_simulation(
    G,
    ...
    输入：
        ...
        threshold: 收敛阈值，当MSE小于此值时认为收敛
        min_steps: 最小步数要求，避免过早收敛
        dist: 用于衡量分布之间的距离的函数，默认为JS散度
        trans_to_itself: 是否允许转移到自身节点，默认不允许
        SEED: 随机数种子

    输出：
        dist_list: 各步的距离列表
        final_samples_count: 最终的样本分布
        final_step: 实际执行的步数
    """
    if SEED is not None:
        np.random.seed(SEED)
    ...
    for step in range(1, max_steps + 1):
        ...
        for node in G.nodes():
            for _ in range(current_samples_count[node]):
                ...
                probabilities = [degree / total_degree for degree in neighbor_degrees]
                # 根据概率分布选择转移到哪个节点
                to_node = random.choices(neighbors, weights=probabilities, k=1)[0]
                new_samples_count[to_node] += 1

        # print(new_samples_count)

        # 计算分布之间的距离
        p = np.array(list(new_samples_count.values()))
        q = np.array(list(current_samples_count.values()))
        p = p / np.sum(p)
        q = q / np.sum(q)
        dist = dist_func(p, q)
        ...
        # 检查收敛条件
        if step > min_steps and dist < threshold:
            # print(f"随机游走在第 {step} 步收敛 (dist < {threshold})")
            return dist_list, new_samples_count, step
        # 更新分布，进行下一轮步数
        current_samples_count = new_samples_count
    # print("达到步数限制，停止随机游走！")
    return dist_list, current_samples_count, max_steps
...
if __name__ == "__main__":
    ...
    G1, G2 = generate_graphs(
        100, p_edge=6 / 100, is_isomorphic=False, is_directed=False, k=6
    )
    n = 20
    initial_samples_count = {node: n for node in G1.nodes()}
    max_steps = 100  # 随机游走的最大步数
    threshold = 2e-5  # 收敛条件：归一化 MSE 阈值

    # 模拟随机游走
    dist_list, final_samples_count, final_step = (
        simulate_random_walks_until_convergence(
            G1, initial_samples_count, max_steps, threshold
        )
    )
    ...
    dist_list, final_samples_count, final_step = (
        simulate_random_walks_until_convergence(
            G2, initial_samples_count, max_steps, threshold
        )
    )
    print(f"收敛分布（经过 {final_step} 步）：", final_samples_count)
